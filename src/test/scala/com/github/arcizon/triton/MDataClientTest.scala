...
class MDataClientTest extends FunSuite with BeforeAndAfterAll with Matchers {
  ...
  override def beforeAll(): Unit = {
    ...
    mdataClient = MDataClient(
      this.getClass.getResource("/mockscripts").getPath.concat("/")
    )
  }
  ...
  test("mdata-list should return a list of metadata keys") {
    assert(
      mdataClient
        .listKeys() === List("root_authorized_keys", "lifecycle", "component")
    )
  }
  ...
  test(
    "mdata-get for metadata key lifecycle should return optional string value as test"
  ) {
    assert(mdataClient.get("lifecycle") === Some("test"))
  }

  test(
    "mdata-get for unavailable metadata key dummy should return optional value None"
  ) {
  ...
  test(
    "mdata-put for metadata key username with arcizon should return boolean true on success"
  ) {
